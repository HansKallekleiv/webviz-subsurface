...
stddev_surf = surfaces.apply(np.var, axis=0)
...
stddev_mesh = surface_to_structured_grid(stddev_surf)
stddev_mesh.elevation()
...
for surface in [min_surf, max_surf]:
    ...
    ziv.append(zif)
stddevvals = [
    stddev_surf.values.flatten(order="F"),
    stddev_surf.values.flatten(order="F"),
]
vol = pv.StructuredGrid(np.array(xiv), np.array(yiv), np.array(ziv))
vol["stddev"] = np.repeat(stddev_surf.values, 2)
vol["Elevation"] = np.array(stddevvals).flatten(order="F")
opacity = (
    pv.opacity_transfer_function(
        [0, 0.0, 0.0, 1.0, 1.0], stddev_mesh["Elevation"].size
    ).astype(float)
    / 255.0
)
# vol.plot(opacity="linear")
...
plot = pv.Plotter()
opacity = [0, 0.0, 0.0, 1.0, 1.0]
plot.add_mesh(vol)  # stddev_mesh["Elevation"])
plot.add_mesh(vol.contour())
plot.show()
...
app.layout = dash_vtk.View(
    ...
    children=[
        ...
        dash_vtk.GeometryRepresentation(
            ...
            # colorMapPreset="BLUE-WHITE",
            # id={"id": self.uuid("well-rep"), "name": well_name},
            children=[
                dash_vtk.Mesh(
                    # id={"id": self.uuid("well-mesh"), "name": well_name},
                    state=to_mesh_state(
                        vol, field_to_keep="Elevation"
                    )  # , field_to_keep="Elevation"),
                ),
            ],
            property={"opacity": 1},
        ),
