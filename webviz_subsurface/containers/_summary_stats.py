...
'''
    ...
    def layout(self):
        return html.Div(children=[
            html.Div(style=SummaryStats.CONTROL_STYLE, children=[
                html.Div([
                    ...
                    dcc.RadioItems(style={'display':'none'},  id=self.radio_plot_type_id,
                                   options=[{'label': i, 'value': i}
            ...
            html.Div(style={'display':'none'}, id='real_selector')
            ])
        ...
        def update_plot(vector, summary_plot_type):
            ...
            if summary_plot_type == 'Statistics':
                return render_stat_plot(
                    self.ensemble_path,
                    self.sampling, vector)
        @app.callback(Output('real_selector', 'children'), [Input(self.chart_id, 'clickData')])
        def current(click_data):
            try:
                real =  click_data['points'][0]['customdata']
            except:
                real = None
            import json
            return json.dumps({'REAL': real})
