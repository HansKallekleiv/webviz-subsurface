...
def update_intersection(
    ...
    @app.callback(
        ...
        State(get_uuid("leaflet-map"), "polyline_points"),
    ...
    def _store_zrange_settings(
        ...
        for option in zrange_locks_options:
            if option["label"] == "Lock range":
                option.update(disabled=(zmin is None or zmax is None))
                # if previous state was not disabled set new value
                if zrange_locks_value == "lock" and (zmin is None or zmax is None):
                    zrange_locks_value = "truncate"

        return zrange_locks_options, zrange_locks_value, zmax, zmin
