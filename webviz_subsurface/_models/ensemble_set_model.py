...
class EnsembleSetModel:
    ...
    def extract_volumes(self, volfolder, volfiles) -> pd.DataFrame:
        ...
        for volname, volfile in volfiles.items():
            df = self._load_csv(csv_file=pathlib.Path(volfolder) / volfile)
            df["SOURCE"] = volname
            self.webvizstore.append(
                (
                    self._load_csv,
                    [{"self": self, "csv_file": pathlib.Path(volfolder) / volfile}],
                )
            )
            dfs.append(df)
